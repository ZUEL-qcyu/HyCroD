...
from models.base_models import LPModel,NCModel
...
def train(args,data):
    ...
    #data = load_data(args, os.path.join(os.environ['DATAPATH']))
    
    args.n_nodes, args.feat_dim = data['features'].shape
    Model = NCModel
    args.n_classes = int(data['labels'].max() + 1)
    args.eval_freq = args.enco_epoch + 1

    if not args.lr_reduce_freq:
        args.lr_reduce_freq = args.enco_epoch
    ...
    for epoch in tqdm(range(args.enco_epoch)):
        tt = time.time()
        model.train()
        optimizer.zero_grad()
